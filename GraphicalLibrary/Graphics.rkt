  ...
  (let* ((show-fps #t)
    ...
    (define frame
      (new closing-frame%
           ...
           [width w]
           [height h]
           [style (list 'no-resize-border)])) ; MODIFIED BY ABEL STUKER
...
(define (make-tile w h [bitmap #f] [mask #f])
  ...
  (let* ((x 0) 
    ...
    (define (draw-text text fontsize x y color underline? bold?)
      (define weight (if bold? 'bold 'normal))
      (when (string? color) (set! color (send the-color-database find-color color)))
      (send bitmap-dc set-font (make-object font% fontsize 'swiss 'normal weight underline?))
      (send bitmap-dc set-text-foreground color)
      (send bitmap-dc draw-text text x y)
      (trigger-update!))

    ;; ADDED BY ABEL STUKER
    ;; Clearing the tile and then Drawing Text that automatically adjusts the width and height of the bitmap to the width and height of the drawn text
    ;; String, Number, Number, Number, (String ∪ Color%) -> void
    (define (draw-tilewrap-text text fontsize x y color underline? bold?)
      (define weight (if bold? 'bold 'normal))
      (send bitmap-dc set-font (make-object font% fontsize 'swiss 'normal weight underline?))
      (define-values (width height x-offset y-offset) (send bitmap-dc get-text-extent text))
      (set! w (max 2 (inexact->exact width)))
      (set! h (max 2 (inexact->exact height)))
      (set! bufferbitmap (make-object bitmap% w h #f #t))
      (set! bitmap-dc (new bitmap-dc%  [bitmap bufferbitmap]))
      (send bitmap-dc draw-bitmap bitmap 0 0)

      (draw-text text fontsize x y color underline? bold?)
      (trigger-update!))
    ...
    (define (dispatch msg . args)
      (cond 
        ...
        ((eq? msg 'draw-tilewrap-text!) draw-tilewrap-text)
...
(define (make-tile-sequence tiles-in)
  ...
  (let ((tiles (if (mlist? tiles-in) (mlist->list tiles-in) tiles-in)) ;; converts mutable list (r5rs) to immutable list (Racket).
    ...
    (define (draw-line x y w h width color)
      (for-each (lambda (tile) ((tile 'draw-line!) x y w h width color  )) tiles) ;; CHANGED BY ABEL STUKER ('draw-line -> 'draw-line!)
      (update-callback))
    ...
    ;; void -> void
    (define (clear)
      (for-each (lambda (tile) (tile 'clear!)) tiles) ;; CHANGED BY ABEL STUKER ('clear -> 'clear!)
      (update-callback))
